...
<!--                 <th>動作</th>
            ...
            <?php 
                try {
                  require_once("php/connect.php");
                  //共幾筆
                  $sql = "select count(*) totalRecords from pho";
                ...
                //$pageRecords
                  ...
                  $sql = "select * from pho join mem using(mem_no) limit $start, $pageRecords";
                  $pho = $pdo->query( $sql);
                  ...
                  for( $i=1; $i<=$pages; $i++){
                    echo "<a href='admin_pho.php?pageNo=$i'> $i</a> &nbsp;&nbsp;&nbsp;";
                  }
                  echo "</td></tr></table>";
...
<script>
$(function(){
  $('.adminMenu li:nth-child(4)').addClass('show');
  $('#photoTable').show().css({'width':'95%'});
})
